...
function Modal({ open, onClose, title, children, wide }: { open:boolean; onClose:()=>void; title:string; children:React.ReactNode; wide?:boolean }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm" onClick={e=>e.target===e.currentTarget&&onClose()}>
      <div className={clsx('bg-white rounded-2xl shadow-2xl w-full animate-fu max-h-[90vh] flex flex-col', wide ? 'max-w-xl' : 'max-w-md')}>
...
export default function DashboardShell({ profile }: { profile: Profile }) {
  ...
  const load = useCallback(async () => {
    const [s, p, sub, sch, f, pay] = await Promise.all([
    ...
    <div className="flex h-screen overflow-hidden">
      ...
      <main className="flex-1 overflow-y-auto bg-gray-50">
        <div className="max-w-6xl mx-auto px-6 py-6">
          {tab === 'home'     && <HomeTab profile={profile} perms={perms} students={students} profiles={profiles} payments={payments} schedules={schedules} subjects={subjects} setTab={setTab}/>}
...
// HOME
// ══════════════════════════════════════════════════════════════
function HomeTab({ profile, perms, students, profiles, payments, schedules, subjects, setTab }:any) {
    ...
    <div className="animate-fu">
        ...
        <>
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
            {[
              ...
              { label:'Teachers', val:teachers.length, icon:GraduationCap, color:'bg-purple-50 text-purple-600', page:'teachers' },
              { label:'Subjects', val:subjects.length, icon:BookOpen, color:'bg-emerald-50 text-emerald-600', page:'subjects' },
              { label:'Classes/week', val:schedules.length, icon:CalendarDays, color:'bg-amber-50 text-amber-600', page:'schedule' },
            ].map(m => {
              const Icon = m.icon
              return (
                <button key={m.label} onClick={()=>setTab(m.page)} className="card p-5 text-left hover:shadow-md transition-shadow group">
                  <div className={clsx('w-10 h-10 rounded-xl flex items-center justify-center mb-3', m.color)}>
          ...
          <div className="grid grid-cols-3 gap-4 mb-6">
            <div className="card p-5 bg-emerald-50 border-emerald-100">
              ...
              <div className="text-2xl font-semibold text-emerald-700">{fmt(paid)}</div>
            </div>
            <div className="card p-5 bg-amber-50 border-amber-100">
              <div className="text-xs text-amber-600 font-medium mb-1">Pending</div>
              <div className="text-2xl font-semibold text-amber-700">{fmt(pending)}</div>
            </div>
            <div className="card p-5 bg-red-50 border-red-100">
              <div className="text-xs text-red-500 font-medium mb-1">Overdue</div>
              <div className="text-2xl font-semibold text-red-600">{overdue.length} students</div>
            </div>
          </div>

          {overdue.length > 0 && (
            <div className="card mb-4">
              <div className="px-5 py-3 border-b border-gray-100 flex items-center gap-2 text-sm font-medium text-red-600">
                <AlertCircle className="w-4 h-4"/> Overdue Fees
              </div>
              {overdue.slice(0,5).map((p:any)=>(
                <div key={p.id} className="flex items-center justify-between px-5 py-3 border-b border-gray-50 last:border-0">
                  <div>
                    <div className="text-sm font-medium text-gray-800">{p.students?.full_name}</div>
                    <div className="text-xs text-gray-400">{p.subjects?.name} · {p.month_label}</div>
                  </div>
      ...
      {isTeacher && (
        <div className="card p-10 text-center">
          ...
          <p className="text-sm text-gray-400 mb-5">As a teacher, you can view classes assigned to your subjects.</p>
    ...
    <div className="animate-fu">
      ...
      <Modal open={open} onClose={()=>setOpen(false)} title={editing?'Edit Student':'Add Student'}>
        <div className="space-y-3">
          ...
          <div><label className="label">Email</label><input className="input" type="email" value={form.email} onChange={e=>setForm(f=>({...f,email:e.target.value}))} placeholder="email@example.com"/></div>
          <div><label className="label">Phone</label><input className="input" value={form.phone} onChange={e=>setForm(f=>({...f,phone:e.target.value}))} placeholder="Mobile"/></div>
          ...
          <div>
            ...
            <div className="space-y-1.5 mt-1">{subjects.map((s:any)=>(
              <label key={s.id} className="flex items-center gap-2 text-sm cursor-pointer">
                <input type="checkbox" checked={form.subject_ids.includes(s.id)} onChange={()=>toggle(s.id)} className="rounded border-gray-300"/>
                {s.name}
              </label>
            ))}</div>
          </div>
          <div className="flex justify-end gap-2 pt-2">
            ...
            <button className="btn-primary" onClick={save} disabled={busy}>{busy?<Loader2 className="w-4 h-4 animate-spin"/>:null}{editing?'Save':'Add Student'}</button>
          </div>
        </div>
      </Modal>
    </div>
...
function TeachersTab({ profiles, subjects, perms, reload }:any) {
  ...
  async function del(id:string) {
    ...
    await fetch(`/api/admin?id=${id}`,{ method:'DELETE' }); reload()
  }

  return (
    <div className="animate-fu">
      <div className="flex items-center justify-between mb-5">
        <div><h1 className="text-xl font-semibold text-gray-900">Teachers</h1><p className="text-sm text-gray-400 mt-0.5">{teachers.length} faculty members</p></div>
            ...
            <div key={t.id} className="card p-5 hover:shadow-md transition-shadow">
              <div className="flex items-start justify-between mb-4">
                <div className="flex items-center gap-3">
                  ...
                  <div><div className="font-medium text-gray-900">{t.full_name}</div><span className="badge bg-emerald-50 text-emerald-700 text-xs">Teacher</span></div>
                </div>
              ...
              <div className="flex flex-wrap gap-1">{tSubs.map((s:any)=><span key={s.id} className={clsx('badge',colorBadge[s.color]||colorBadge.violet)}>{s.name}</span>)}{!tSubs.length&&<span className="text-xs text-gray-300">None assigned</span>}</div>
            </div>
          )
        })}
      ...
      <Modal open={open} onClose={()=>setOpen(false)} title="Add Teacher">
        <div className="space-y-3">
          ...
          <div className="flex justify-end gap-2 pt-2">
            <button className="btn" onClick={()=>setOpen(false)}>Cancel</button>
            <button className="btn-primary" onClick={save} disabled={busy}>{busy?<Loader2 className="w-4 h-4 animate-spin"/>:null}Add Teacher</button>
          </div>
        </div>
      </Modal>
    </div>
...
function SubjectsTab({ subjects, profiles, students, fees, perms, reload }:any) {
  const supabase = sb()
  const teachers = profiles.filter((p:any)=>p.role==='teacher'||p.role==='center_manager'||p.role==='superadmin')
  const [open, setOpen] = useState(false)
  const [editing, setEditing] = useState<any>(null)
...
function ScheduleTab({ schedules, subjects, students, profile, perms, reload }:any) {
  const supabase = sb()
  const isTeacher = profile.role === 'teacher'
  // Teacher sees only their subjects' schedules
      ...
      <div className="card overflow-hidden mb-5">
        <div className="overflow-x-auto">
          <table className="w-full text-xs" style={{minWidth:600}}>
            <thead>
              <tr>
                <th className="th w-16 bg-gray-50">Time</th>
                {DAYS.map(d=><th key={d} className="th bg-gray-50 text-center">{d}</th>)}
              </tr>
            </thead>
            <tbody>
              {TIMES.map(time=>(
                <tr key={time}>
                  ...
                  {DAYS.map(day=>{
                    ...
                    return (
                      <td key={day} className="td align-top" style={{minHeight:40}}>
                        {cls.map((c:any)=>{ const sub = subById(c.subject_id); return sub ? <div key={c.id} className={clsx('rounded px-1.5 py-1 mb-0.5 text-xs font-medium',colorCell[sub.color]||colorCell.violet)}>{sub.code} {c.duration_minutes}m</div> : null })}
                      </td>
                    )
                  })}
                </tr>
      ...
      {!isTeacher&&(
        <Modal open={open} onClose={()=>setOpen(false)} title="Schedule Class" wide>
          <div className="space-y-3">
            ...
            <div><label className="label">Students</label>
              <div className="grid grid-cols-2 gap-1.5 mt-1 max-h-40 overflow-y-auto">{students.map((s:any)=>(
                <label key={s.id} className="flex items-center gap-2 text-sm cursor-pointer"><input type="checkbox" checked={form.student_ids.includes(s.id)} onChange={()=>setForm(f=>({...f,student_ids:f.student_ids.includes(s.id)?f.student_ids.filter(x=>x!==s.id):[...f.student_ids,s.id]}))} className="rounded border-gray-300"/>{s.full_name}</label>
              ))}</div>
            </div>
...
function FeesTab({ subjects, fees, perms, reload }:any) {
  ...
  const openEdit = (sub:any) => {
    ...
    setEditSub(sub); setForm({ amount:f?String(f.amount):'', frequency:f?.frequency||'Monthly', due_day:f?String(f.due_day):'5' }); setOpen(true)
  }

  async function save() {
    if (!form.amount||!editSub) return; setBusy(true)
    const payload = { subject_id:editSub.id, amount:+form.amount, frequency:form.frequency, due_day:+form.due_day }
    const existing = feeFor(editSub.id)
    if (existing) await supabase.from('fee_structures').update(payload).eq('id',existing.id)
  ...
  return (
    <div className="animate-fu">
      ...
      <div className="card overflow-hidden">
        <table className="w-full">
          <thead><tr><th className="th">Subject</th><th className="th">Fee</th><th className="th">Frequency</th><th className="th">Due Day</th><th className="th w-28"></th></tr></thead>
          <tbody>
            {subjects.map((s:any)=>{
              const f = feeFor(s.id)
              return (
                <tr key={s.id} className="hover:bg-gray-50/50">
                  ...
                  <td className="td font-semibold">{f?fmt(f.amount):<span className="text-gray-300">Not set</span>}</td>
                  <td className="td text-gray-400">{f?.frequency||'—'}</td>
                  <td className="td text-gray-400">{f?`Day ${f.due_day}`:'—'}</td>
                  <td className="td"><button onClick={()=>openEdit(s)} className="btn btn-sm"><Edit className="w-3 h-3"/> {f?'Edit':'Set'}</button></td>
                </tr>
              )
            })}
          </tbody>
      ...
      <Modal open={open} onClose={()=>setOpen(false)} title={`Fee — ${editSub?.name}`}>
        <div className="space-y-3">
          <div><label className="label">Amount (₹) *</label><input className="input" type="number" value={form.amount} onChange={e=>setForm(f=>({...f,amount:e.target.value}))} placeholder="2500"/></div>
          <div className="grid grid-cols-2 gap-3">
          ...
          <div className="flex justify-end gap-2 pt-2"><button className="btn" onClick={()=>setOpen(false)}>Cancel</button><button className="btn-primary" onClick={save} disabled={busy}>{busy?<Loader2 className="w-4 h-4 animate-spin"/>:null}Save Fee</button></div>
        </div>
      </Modal>
    </div>
  )
              ...
              {filtered.map((p:any)=>(
                <tr key={p.id} className="hover:bg-gray-50/50">
                  ...
                  <td className="td"><div className="flex gap-1">
                    {p.status!=='paid'&&perms.managePayments&&(<>
                      <button onClick={()=>markPaid(p.id)} className="btn btn-sm" title="Mark paid"><CheckCircle className="w-3.5 h-3.5 text-emerald-500"/></button>
                      <button onClick={()=>setReminder(p)} className="btn btn-sm" title="Reminder"><Bell className="w-3.5 h-3.5 text-amber-500"/></button>
                    </>)}
      ...
      <Modal open={open} onClose={()=>setOpen(false)} title="Record Payment">
        <div className="space-y-3">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div><label className="label">Amount (₹) *</label><input className="input" type="number" value={form.amount} onChange={e=>setForm(f=>({...f,amount:e.target.value}))} placeholder="Auto-fills"/></div>
          </div>
        ...
        <Modal open={!!invoice} onClose={()=>setInvoice(null)} title="Invoice">
          <div className="border border-gray-100 rounded-xl p-5 space-y-4">
            <div className="flex justify-between">
              <div><div className="text-lg font-bold text-brand-500">Academy LMS</div><div className="text-xs text-gray-400">Payment Receipt</div></div>
              <div className="text-right"><div className="font-mono text-sm font-semibold">INV-{invoice.id.slice(0,6).toUpperCase()}</div><div className="text-xs text-gray-400">{invoice.payment_date}</div></div>
            </div>
            <div className="bg-gray-50 rounded-lg p-3 text-sm">
              <div className="font-medium">{invoice.students?.full_name}</div>
              <div className="text-xs text-gray-400">{invoice.students?.email} · {invoice.students?.phone}</div>
            </div>
            <table className="w-full text-sm">
              <thead><tr className="border-b border-gray-100"><th className="text-left text-xs text-gray-400 pb-2 font-normal">Description</th><th className="text-left text-xs text-gray-400 pb-2 font-normal">Period</th><th className="text-right text-xs text-gray-400 pb-2 font-normal">Amount</th></tr></thead>
              <tbody><tr><td className="py-3">{invoice.subjects?.name} — Tuition</td><td className="py-3 text-gray-400">{invoice.month_label}</td><td className="py-3 text-right font-semibold">{fmt(invoice.amount)}</td></tr></tbody>
            </table>
            <div className="flex justify-between items-center pt-2 border-t border-gray-100">
              <span className="badge bg-emerald-50 text-emerald-700">PAID ✓</span>
              <div className="text-right"><div className="text-xs text-gray-400">Total</div><div className="text-xl font-bold text-brand-500">{fmt(invoice.amount)}</div></div>
            </div>
      ...
      {reminder&&(
        <Modal open={!!reminder} onClose={()=>setReminder(null)} title="Payment Reminder">
          <div className="bg-amber-50 border border-amber-100 rounded-xl p-4 mb-4 text-sm leading-relaxed text-gray-700">
            ...
            Please pay at the earliest to avoid disruption to classes.<br/><br/>
            — Academy Management
          </div>
          <div className="text-xs text-gray-400 mb-4">To: {reminder.students?.email} · {reminder.students?.phone}</div>
          <div className="flex justify-end gap-2">
            <button className="btn" onClick={()=>setReminder(null)}>Cancel</button>
            <button className="btn-primary" onClick={()=>{ alert('Reminder sent! (Connect Twilio/Resend in production)'); setReminder(null) }}><Bell className="w-4 h-4"/> Send Reminder</button>
          </div>
        </Modal>
      )}
    </div>
...
function UsersTab({ profiles, profile: self, perms, reload }:any) {
  ...
  async function del(id:string) {
    if (id===self.id) return alert("Can't delete yourself")
    if (!confirm('Delete user?')) return
    await fetch(`/api/admin?id=${id}`,{ method:'DELETE' }); reload()
  }

  async function changeRole(id:string, role:string) {
    if (id===self.id) return alert("Can't change your own role")
    await fetch('/api/admin',{ method:'PATCH', headers:{'Content-Type':'application/json'}, body:JSON.stringify({ id, role }) }); reload()
  }

  const roleAccess: Record<Role,string> = {
    superadmin: 'All modules + user management',
    center_manager: 'Students, teachers, schedule, fees, payments',
    teacher: 'Own schedule only',
  }
        ...
        {(['superadmin','center_manager','teacher'] as Role[]).map(r=>{
          ...
          return (
            <div key={r} className="card p-4">
              <span className={clsx('badge mb-2',ROLE_COLOR[r])}>{ROLE_LABEL[r]}</span>
              <div className="text-2xl font-semibold text-gray-900 mb-0.5">{count}</div>
              <div className="text-xs text-gray-400">{roleAccess[r]}</div>
            </div>
          )
        })}
      </div>
          ...
          <tbody>
            {profiles.map((u:any,i:number)=>(
              <tr key={u.id} className={clsx('hover:bg-gray-50/50', u.id===self.id&&'bg-brand-50/20')}>
                ...
                <td className="td">
                  {u.id!==self.id ? (
                    <select className="input text-xs py-1 w-40" value={u.role} onChange={e=>changeRole(u.id,e.target.value)}>
                      <option value="superadmin">Super Admin</option>
                      <option value="center_manager">Center Manager</option>
                      <option value="teacher">Teacher</option>
                    </select>
                  ) : <span className="text-xs text-gray-300">Own account</span>}
                </td>
